       ...
       public void DrawLine(Vertex v0, Vertex v1, Color color)
{
    ...
    for (int i = 0; i <= steps; i++)
    {
        ...
        public void RenderSceneFil(Scene scene, int filter)
        {
            canvas.FastClear(); // Clears the canvas
            ...
            switch (filter)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    BP.Binary(canvas.bits);
                    canvas.Refresh();
                    break;
                case 4:
                    BP.Invert(canvas.bits);
                    canvas.Refresh();
                    break;
                case 5:
                    canvas.ApplyFilter(BP.ApplyConvolutionalFilter, new float[(int)canvas.Width, (int)canvas.Height]);
                    canvas.Refresh();
                    break;
                default:
                    canvas.Refresh();
